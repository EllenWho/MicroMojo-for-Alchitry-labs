...
int g_ExternalSensor = 0; // Modified by Ziyi Hu
int g_Servo = 1;
int g_Monitoring = 0;																										//////
...
int PIZStage::Initialize()
{
 ...
 /*  // Internal sensor position
   ...
   */
   ...
   //////////////////////////////// Servo on property		Modified by Ziyi Hu																						//////
   pAct = new CPropertyAction(this, &PIZStage::OnServoState);
   CreateProperty("Servo", "0", MM::Integer, false, pAct);
   AddAllowedValue("Servo", "1");
   AddAllowedValue("Servo", "0");
...
//Modified by Ziyi Hu
int PIZStage::OnServoState(MM::PropertyBase* pProp, MM::ActionType eAct)																						/////
{
	if (eAct == MM::BeforeGet)
	{
		pProp->Set((long)g_Servo);
	}
	else if (eAct == MM::AfterSet)
	{
		long pos;
